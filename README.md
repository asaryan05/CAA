...
"Hello, World!" is a constant string literal, so the compiler emitted it verbatim into .rdata.
std::cout << just receives a pointer to that location at runtime — it does not validate the bytes.
We replaced the 13 bytes in place, so the pointer in .text is still valid and points at the new string.
...
Defines the input file, the output file, the string to find, and its replacement.
Length guard. Refuses to run unless both strings are the same byte length. A longer replacement would shift every byte after the patch and break pointers throughout the executable.
Reads the entire hello.exe into a byte array.
Decodes those bytes as ASCII text purely so .IndexOf() can find the pattern's byte offset.
Bails out if the string isn't present.
Overwrites the 13 bytes at the located offset with the ASCII bytes of the replacement string, in place.
Writes the modified byte array to hello_modified.exe. The original hello.exe is untouched.
Limitations

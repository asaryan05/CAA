...
Port scanning (list active connections)
Port blocking (record blocked ports per direction)
Port unblocking
Traffic analysis (count usage, surface "heavy" ports)
2. Project layout
lab13-dev/
...
Heavy ports — bar chart of the top-N busiest ports. Bars colored red if the port appears in blocked_in or blocked_out, blue otherwise.
Protocol distribution — pie chart of HTTP / HTTPS / FTP / SSH / Telnet / TCP-UDP across all open ports.
Connection status — bar chart of LISTEN / ESTABLISHED / TIME_WAIT / CLOSE_WAIT / NONE / ... counts.
Firewall rules summary — text panel listing the blocked-IN and blocked-OUT sets, plus totals.
...
6. Typical workflow
Run firewall-1.py — produces a firewall_log_<timestamp>.json snapshot of open ports, heavy ports, and the in-memory rule set.
Run visualize.py — produces firewall_chart_<timestamp>.png from that log.
(Optional) Run one of the C++ binaries on the matching OS to actually install rules into iptables or Windows Firewall.
7. Lab goals
